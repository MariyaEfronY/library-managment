...
import Request, { IRequest } from "@/models/Request"; // Assuming you have IRequest export
...
export async function PATCH(
  req: NextRequest,
  // FIX: Correctly destructure params to avoid the Promise/sync error
  ...
  try {
    ...
    if (status === "approved") {
      ...
      // a) Decrement Inventory (FindByIdAndUpdate is safer in a transaction)
      ...
      await IssueRecord.create(
        [
          {
            studentId: request.requestedBy, // Use the ID from the request model
    ...
    // --- REJECTION LOGIC ---
        ...
        request.returnDate = undefined; // Clear date on rejection
    }
